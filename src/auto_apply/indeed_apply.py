...
class IndeedAutoApply(BaseAutoApply):
    """Indeed-specific auto-apply implementation with CAPTCHA detection"""
    
    def __init__(self, headless: bool = False):
        super().__init__("indeed", headless)
    
    def apply_to_job(self, job_url: str, user_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Apply to Indeed job with CAPTCHA handling
        ...
        try:
            ...
            try:
                apply_button = self.wait.until(
                    EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Apply now') or contains(., 'Apply')]"))
                )
                ...
                logger.info(f"Initiated Indeed application for {job_url}")
                
                return {
                    'status': 'success',
                    'message': 'Indeed application initiated. Please complete in the browser.',
